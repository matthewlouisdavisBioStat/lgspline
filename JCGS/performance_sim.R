...
sigmas <- c(0.5, 2.0)
sp_fixed <- 1e-4
time_cap <- 3600 # one hour time cap
collapse_tol <- 1e-4
n_cores <- 12
base_seed <- 1234
tprs_max_knots <- 1000  # mgcv default subsample cap for the tprs basis is 2000
...
#  q = 2: Peaks function on [-3, 3]^2.
#  q = 4: Friedman 3 (Friedman 1991), arctan of a ratio of four predictors
#         with incommensurate ranges.
generate_data <- function(q, n, sigma) {
  if (q == 1) {
    ...
    t1 <- runif(n, -3, 3)
    t2 <- runif(n, -3, 3)
    f_true <- 3 * (1 - t1)^2 * exp(-t1^2 - (t2 + 1)^2) -
    ...
    t1 <- runif(n, 0, 100)
    t2 <- runif(n, 40 * pi, 560 * pi)
    t3 <- runif(n, 0, 1)
    t4 <- runif(n, 1, 11)
    f_true <- atan((t2 * t3 - 1 / (t2 * t4)) / t1) * 10
    T_mat <- cbind(t1, t2, t3, t4)
...
## Detect when all partitions collapsed to a single polynomial in lgspline
coefficients_collapsed <- function(fit, tol) {
  median(apply(Reduce('cbind', coef(fit)), 1, sd)) < tol
}

################################################################################

## ## lgspline Fit (with adaptive constraint relaxation) ## ##
fit_lgspline <- function(T_train, y_train, f_true, q, tuned, k_per) {
  ...
  if (!is.null(fit) && coefficients_collapsed(fit, collapse_tol)) {
    constraint_level <- "no_second_deriv"
    run2 <- timed_fit(function() {
      lgspline(predictors = T_train, y = y_train, K = k_per * q, opt = tuned,
               ...
               return_varcovmat = FALSE, return_G = FALSE,
               return_Ghalf = FALSE, return_U = FALSE,
               estimate_dispersion = FALSE, include_warnings = FALSE)
    })
    fit <- run2$result
    run$elapsed <- run$elapsed + run2$elapsed

    if (!is.null(fit) && coefficients_collapsed(fit, collapse_tol)) {
      constraint_level <- "no_first_deriv"
      run3 <- timed_fit(function() {
        lgspline(predictors = T_train, y = y_train, K = k_per * q, opt = tuned,
      ...
      fit <- run3$result
      run$elapsed <- run$elapsed + run3$elapsed
    }
  }

  if (is.null(fit))
    return(list(rmse = NA, time = run$elapsed,
                constraint_level = constraint_level))
  yhat <- tryCatch(predict(fit), error = function(e) rep(NA, length(y_train)))
  list(rmse = sqrt(mean((yhat - f_true)^2, na.rm = TRUE)),
       time = run$elapsed, constraint_level = constraint_level)
}
...
#  Predictors are standardized so isotropy is meaningful when units differ.
...
fit_tp <- function(T_train, y_train, f_true, q, tuned, k_per) {
  ## Standardize so the isotropic penalty is sensible for heterogeneous units
  ctr <- colMeans(T_train)
  scl <- apply(T_train, 2, sd)
  scl[scl == 0] <- 1
  T_std <- scale(T_train, center = ctr, scale = scl)

  df <- as.data.frame(T_std)
...
make_trunc_basis <- function(t_vec, knots, degree = 4) {
...
build_trunc_design <- function(T_mat, K_knots, degree = 4) {
...
run_one_rep <- function(args) {
  ...
  for (tuned in c(TRUE, FALSE)) {
    ...
    res_lg <- tryCatch(
      fit_lgspline(train$T_mat, train$y, train$f_true, q, tuned, k_per),
      error = function(e) list(rmse = NA, time = NA, constraint_level = NA))

    res_cr <- tryCatch(
      fit_cr(train$T_mat, train$y, train$f_true, q, tuned, k_per),
      error = function(e) list(rmse = NA, time = NA))

    res_tp <- tryCatch(
      fit_tp(train$T_mat, train$y, train$f_true, q, tuned, k_per),
      error = function(e) list(rmse = NA, time = NA))

    res_tr <- tryCatch(
      fit_trunc_poly(train$T_mat, train$y, train$f_true, q, tuned, k_per),
      error = function(e) list(rmse = NA, time = NA))
    ...
    rows[[length(rows) + 1]] <- data.frame(
      ...
      constraint_level = c(res_lg$constraint_level, NA, NA, NA),
      stringsAsFactors = FALSE)
  }
  do.call(rbind, rows)
}
...
for (i in seq_len(nrow(scenarios))) {
  for (b in seq_len(B_reps)) {
    tasks[[length(tasks) + 1]] <- c(as.list(scenarios[i, ]), list(rep = b))
  }
}
...
clusterExport(cl, c("tasks", "task_seeds", "run_one_rep",
                    ...
                    "timed_fit", "coefficients_collapsed", "collapse_tol",
                    "time_cap", "sp_fixed",
...
save(results, file = "performance_results.RData")
...
constraint_tab <- results %>%
  filter(method == "lgspline", !is.na(constraint_level)) %>%
  group_by(q, N, sigma, K, tuning, constraint_level) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(q, N, sigma, K, tuning) %>%
...
print(as.data.frame(constraint_tab))
...
## lgspline
fit_lg <- lgspline(predictors = dop$T_mat, y = y_vec, K = 15,
                   return_varcovmat = FALSE, return_G = FALSE,
                   return_Ghalf = FALSE, return_U = FALSE,
                   estimate_dispersion = FALSE,
                   verbose = FALSE, include_warnings = FALSE)
pred_lg <- as.numeric(predict(fit_lg, newdata = t_grid_mat))

## Cubic RS (mgcv, smoothing spline penalty via bs="cr")
df_sim <- data.frame(t1 = t_vec, y = y_vec)
fit_cr_dop <- gam(y ~ s(t1, bs = 'cr', k = 15), data = df_sim, method = 'REML')
pred_cr <- predict(fit_cr_dop, newdata = data.frame(t1 = t_grid))

## TPRS (mgcv, isotropic thin plate via bs="tp"); 1D so just a single smoother
fit_tp_dop <- gam(y ~ s(t1, bs = 'tp', k = 15), data = df_sim, method = 'REML')
pred_tp <- predict(fit_tp_dop, newdata = data.frame(t1 = t_grid))

## Truncated polynomial: stack training + grid so knots are shared
t_stack <- rbind(dop$T_mat, t_grid_mat)
des_stack <- build_trunc_design(t_stack, K_knots = 15, degree = 4)
X_stack <- des_stack$X
p <- ncol(X_stack)
