...
K_vals <- c(100, 250, 500, 750, 1000) - 1
n_cores <- 12
B_reps <- 5

## Common fit arguments with default parallel settings
fit_args <- list(
  opt = FALSE,
...
for (K in K_vals) {
  ...
  for (b in 1:B_reps) {
    args <- c(list(predictors = predictors_train, y = response_train, K = K, 
                   cl = cl),
              fit_args)
    tm <- system.time({
      res <- safe_predict(
...
print(session_info)
save(results, session_info, file = 'volcano_results.RData')
...
## Propagate SD via the ratio: sd(seq/par) ~= |seq/par| * sqrt((sd_s/mean_s)^2 + 
#  (sd_p/mean_p)^2)
...
plot(results$partitions, results$speedup_mean,
     ...
     ylim = c(0, max(results$speedup_mean + results$speedup_sd, na.rm = TRUE) * 
                1.15),
     cex = 1.5,
     cex.lab = 1.3,
     cex.axis = 1.1,
