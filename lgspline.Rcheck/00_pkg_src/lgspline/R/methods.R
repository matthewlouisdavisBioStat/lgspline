...
#' \eqn{N_\mathrm{new} \times \code{num_draws}} matrix.
#'
#' @examples
#' set.seed(1234)
...
generate_posterior_correlation <- function(
  ...
  #     These are all fixed across draws. The key insight is that we
  #     do NOT re-extract or recompute B_raw, B, or ytilde.
  ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
  K  <- object$K
...
equation.lgspline <- function(object,
  ...
  if(is.null(predictor_names)){
    ## Try to get from raw_expansion_names
    raw_nms <- object$raw_expansion_names
    if(!is.null(raw_nms) && length(object$power1_cols) > 0){
      predictor_names <- raw_nms[object$power1_cols]
    } else if(!is.null(raw_nms) && length(object$nonspline_cols) > 0){
      predictor_names <- raw_nms[object$nonspline_cols[1:min(q, length(object$nonspline_cols))]]
    } else {
      predictor_names <- paste0("x", 1:q)
    }
  }
  if(length(predictor_names) < q){
    predictor_names <- c(predictor_names,
                         paste0("x", (length(predictor_names)+1):q))
  }

  ## Resolve calculus mode and target variable.
  #  Priority: first_derivative > second_derivative > antiderivative.
  #  Only one variable at a time is supported; warn if the user supplies
  #  a vector of length > 1 for any of these arguments.
  calc_mode <- "equation"
  calc_var_idx <- NULL
  calc_var_name <- NULL

  ## Helper: resolve a variable specification to a 1-based integer index
  #  and a display name. Accepts character name or integer index.
  ...
  ## Resolve response name based on link function
  fam <- object$family
  ...
  ## Parse each coefficient name into a structured representation:
  #  list of (predictor_index, power) pairs. The intercept has no
  #  predictor entries. This is the foundation for symbolic differentiation
  #  and integration.
  #
  #  Expansion name conventions from get_polynomial_expansions:
  #    "intercept"       -> constant term
  #    "_j_"             -> linear in predictor j
  #    "_j_^2"           -> quadratic in predictor j
  #    "_j_^3"           -> cubic in predictor j
  #    "_j_^4"           -> quartic in predictor j
  #    "_j_x_k_"         -> interaction: linear j * linear k
  #    "_j_x_k_^2"       -> interaction: linear j * quadratic k
  #    "_j_x_k_x_l_"     -> three-way interaction
  #
  #  The internal column naming uses "_j_" where j is the 1-based column
  #  index. We parse these to extract which predictors appear and at what
  #  power.

  ## Map from internal "_j_" notation to predictor index
  .parse_term <- function(nm){
    if(nm == "intercept" || grepl("^intercept$", nm, ignore.case = TRUE)){
      return(list(is_intercept = TRUE, factors = list()))
    }

    ## Split on "_x_" to get individual variable tokens
    tokens <- strsplit(nm, "_x_")[[1]]
    factors <- lapply(tokens, function(tok){
      ## Check for power suffix: "_j_^n" or just "_j_"
      ...
      ## Extract predictor index from "_j_" pattern
      idx_match <- regmatches(base_tok, regexec("^_([0-9]+)_$", base_tok))[[1]]
      if(length(idx_match) == 2){
        pred_idx <- as.integer(idx_match[2])
      } else {
        ## Fallback: try matching against predictor_names
  ...
  ## Symbolic calculus on a single term.
  #  Given coefficient value, parsed term structure, target variable index,
  #  and operation mode, return the transformed (new_coef, new_term_parts)
  #  or NULL if the term vanishes.
  #
  #  For differentiation w.r.t. x_j:
  #    - If x_j does not appear in the term, derivative is 0 (return NULL).
  #    - If x_j appears at power p, multiply coef by p, reduce power by 1.
  #      If power becomes 0, remove that factor from the term.
  #
  #  For second derivative: apply differentiation twice.
  #
  #  For antiderivative w.r.t. x_j:
  #    - If x_j appears at power p, increase power by 1, divide coef by (p+1).
  #    - If x_j does not appear, multiply the entire term by x_j (power 1),
  #      i.e. coef stays the same, append factor (j, 1).

  .differentiate_term <- function(coef_val, parsed, var_idx){
    if(parsed$is_intercept) return(NULL)

    ## Find which factor (if any) involves var_idx
    hit <- NULL
    hit_pos <- 0L
  ...
  .antidifferentiate_term <- function(coef_val, parsed, var_idx){
    if(parsed$is_intercept){
      ## Integral of constant C w.r.t. x_j = C * x_j
    ...
    if(hit_pos > 0){
      ...
      ## Variable not present: multiply by x_j
      new_coef <- coef_val
  ...
  ## Reconstruct a human-readable term name from parsed structure.
  .rebuild_term_name <- function(parsed, predictor_names){
    if(parsed$is_intercept) return("intercept")
    parts <- sapply(parsed$factors, function(f){
      if(is.na(f$pred_idx)){
        nm <- "?"
      } else {
  ...
  ## Format one coefficient-term pair for printing.
  .fmt_coef <- function(val, term_nm, first_term = FALSE){
    ...
    ## Handle intercept/constant term
    if(term_nm == "intercept" || term_nm == ""){
      if(first_term){
        return(val_str)
      } else {
        if(val >= 0){
          return(paste0(" + ", val_str))
        } else {
    ...
    ## Build coefficient * term string
    if(first_term){
      if(abs(val - 1) < .Machine$double.eps * 100){
        return(term_nm)
      } else if(abs(val + 1) < .Machine$double.eps * 100){
        return(paste0("-", term_nm))
      } else {
      ...
      if(val >= 0){
        ...
        if(abs(val + 1) < .Machine$double.eps * 100){
          ...
          return(paste0(" - ", gsub("^-", "", val_str), "*", term_nm))
        }
      }
    }
  }

  ## Build the left-hand side label based on calculus mode.
  ...
  #  applying the requested calculus operation.
  formulas <- character(K + 1)
  for(k in 1:(K + 1)){
    coefs <- object$B[[k]]
    nms   <- names(coefs)
    if(is.null(nms)) nms <- object$raw_expansion_names
    ...
    for(i in seq_along(coefs)){
      orig_coef <- coefs[i]
      orig_nm   <- nms[i]
      parsed    <- .parse_term(orig_nm)

      if(calc_mode == "equation"){
        ## No calculus: use original coefficient and name
        #  Clean up term name: replace _j_ with predictor name
  ...
  ## Build partition boundary info.
  #  1D: interval bounds on predictor scale from knot locations.
  #  Multi-D: use knot midpoints (stored in make_partition_list$knots)
  bounds_info <- NULL
  if(show_bounds && K > 0){
    if(q == 1 && !is.null(object$knots)){
      ## 1D case: interval bounds on predictor scale
      ...
      ## Multi-D case: attempt to derive per-partition boundaries from
      #  knot midpoints or cluster centers.
      #
      #  The make_partition_list$knots matrix stores midpoints between
      #  adjacent cluster centers (one row per pair of neighboring
      #  partitions per dimension). These define the approximate Voronoi
      #  boundaries in each coordinate. For each partition, we find
      #  the nearest knot midpoints along each dimension to define
      #  the bounding box.
      #
      #  When knots are not available, fall back to printing centers.
      centers <- object$make_partition_list$centers
      knots_mp <- object$make_partition_list$knots

      if(!is.null(centers)){
        n_centers <- nrow(centers)
        n_dims <- ncol(centers)

        ## Resolve column names for centers/bounds display
        ...
        ## Recover training predictor ranges per dimension
        all_linear_cols <- sort(c(object$power1_cols, object$nonspline_cols))
        training_ranges <- matrix(NA, 2, min(q, n_dims))
        rownames(training_ranges) <- c("min", "max")
        for(jj in seq_len(min(q, n_dims))){
        ...
        ## Build per-partition bounding boxes.
        #  For each partition (cluster center), find the closest knot
        #  midpoints in each dimension to define lower/upper bounds.
        #  If knots are unavailable, compute midpoints between sorted
        #  center coordinates per dimension.
        ...
        for(jj in seq_len(n_dims)){
          ...
          ## Compute midpoints between consecutive sorted centers
          ...
          ## Assign lower/upper for each partition in sorted order
          for(rank in seq_along(ord_j)){
            part_idx <- ord_j[rank]
            if(rank == 1){
              lo <- if(!is.na(training_ranges[1, jj])){
            ...
            if(rank == length(ord_j)){
              hi <- if(!is.na(training_ranges[2, jj])){
        ...
        ## Package as a list of lower/upper matrices
        bounds_info <- list(
          lower = bounds_lower,
          upper = bounds_upper,
          centers = centers,
          colnames = center_colnames
        )
      }
    }
  }

  ## Print header and equations.
  cat("\n")
  cat("================================================================================\n")
  if(calc_mode == "equation"){
    cat("Fitted Equations: lgspline Model\n")
  } else if(calc_mode == "first_derivative"){
    cat("First Derivative w.r.t. ", calc_var_name, ": lgspline Model\n", sep = "")
  } else if(calc_mode == "second_derivative"){
    cat("Second Derivative w.r.t. ", calc_var_name, ": lgspline Model\n", sep = "")
  } else if(calc_mode == "antiderivative"){
    cat("Antiderivative w.r.t. ", calc_var_name, ": lgspline Model\n", sep = "")
  }
  cat("================================================================================\n")
  ...
  ## Print equations with bounds
  for(k in 1:(K + 1)){
    cat("\n")
    cat("Partition", k, "\n")

    ## Print bounds for this partition
    if(show_bounds && !is.null(bounds_info)){
      if(q == 1 && is.matrix(bounds_info) && ncol(bounds_info) == 2){
        ## 1D interval bounds
        lb <- bounds_info[k, 1]
        ub <- bounds_info[k, 2]
        lb_str <- if(is.finite(lb)) format(round(lb, digits), nsmall = digits) else "-Inf"
        ub_str <- if(is.finite(ub)) format(round(ub, digits), nsmall = digits) else "+Inf"

        ## Bracket notation: first partition gets [, last gets ]
        ...
        ## Multi-D bounding box from knot midpoints
        ...
        for(jj in seq_len(n_dims)){
          ...
          ## First partition along this dimension gets [, last gets ]
          ## Use ( ) for interior boundaries
  ...
  if(!is.null(object$sigmasq_tilde) && is.finite(object$sigmasq_tilde)){
    cat("Dispersion (sigma^2):", format(object$sigmasq_tilde, digits = digits), "\n")
  }
  if(!is.null(object$trace_XUGX) && is.finite(object$trace_XUGX)){
    cat("Effective df:", format(object$N - object$trace_XUGX, digits = 2), "\n")
  }
  cat("================================================================================\n")
  cat("\n")
...
print.equation <- function(x, ...) {
  ## For objects returned by equation() called with assignment
  cat("\nFitted Equations")
  if(!is.null(x$mode) && x$mode != "equation"){
    mode_label <- switch(x$mode,
